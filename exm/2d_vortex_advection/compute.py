...
with_grid   = [588,588]   # number of points in each direction
...
# ... as fast as possible!
with_proc = [6,4] # mpi proc. topology
...
for i in range(nx+2*hlo):
    for j in range(ny+2*hlo):
        ...
        v[i,j]   = Vinf + dpsidx
        pij      = p0 + pf 
        et[i,j]  = eos_e(r0,pij) + dn.cst(0.5)*(u[i,j]**2 + v[i,j]**2)
...
t1 = timer()
if dMpi.ioproc: print('Total time:', t1-t0)
if dMpi.ioproc: print('Reduced time:', (t1-t0)/(grid['nxgb']*grid['nygb']*grid['nzgb']*3*nitmax)*mpi['nxpr']*mpi['nypr']*mpi['nzpr']  )
