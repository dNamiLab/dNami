...
SW: Edited for spherical boundary conditions in 2D
"""
...
class type_mpi:
    ...
    #
    def swap(self, f, nh, tree, spherical=True):
        """
        ...
        """
        # 
        self.swapXc(f, nh, tree, spherical=spherical)
        ndim = tree['eqns']['ndim']
        if ndim > 1:
...
# Swap with corners:
    ...
    def swapXc(self, f, nh, tree,spherical=True):
        """
        Swaps array "f(X,0:nv)" (a function of position vector X e.g. X=(x,y,z)) in the x-direction
        ...
        if ndim == 1:
            ...
            if self.flipped :
                ...
                buf = f[nx:nh + nx, 0:2 * nh + ny,
                        0:nv].copy().reshape((nh * (ny + 2 * nh) * nv, 1))
            #
            if self.reverse and self.edge:
                buf = buf.reshape((nh, ny + 2 * nh, nv))
                for inv in range(nv):
                    buf[:,:,inv] = np.fliplr(buf[:,:,inv])
                    buf[:,:,inv] = np.flipud(buf[:,:,inv])
                buf = buf.reshape((nh * (ny + 2 * nh) * nv, 1))  
            if self.sym and spherical:
                buf = buf.reshape((nh, ny + 2 * nh, nv))
                for inv in self.velidx:
                    buf[:,:,inv] = -buf[:,:,inv]
                buf = buf.reshape((nh * (ny + 2 * nh) * nv, 1))  
            #
            if self.iMpi:
