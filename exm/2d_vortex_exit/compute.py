...
# -- Parameters
r0 = dn.cst(1.)                             #baseflow density
p0 = dn.cst(1.)                             #baseflow pressure 
c0 = np.sqrt( dn.cst(1. + delta)*p0/r0  )   #speed of sound 
Rc = dn.cst(0.05)*L                         #vortex radius
Ma = dn.cst(0.5)                            #flow mach number
...
yl     = x[dMpi.jbeg-1:dMpi.jend+2*hlo]

XX, YY = xl[:,np.newaxis], yl[np.newaxis,:]
