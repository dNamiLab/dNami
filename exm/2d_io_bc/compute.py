...
# ... in space ...
with_length = [1.,1.]      # domain length in each direction
with_grid   = [64,64]   # number of points in each direction
...
a[dom] = np.random.rand(nx+2*hlo,ny+2*hlo) 
b[dom] = np.random.rand(nx+2*hlo,ny+2*hlo)
...
if dMpi.ioproc:
    print('Diff in core:')
dn.dnami_io.globalMinMax(dtree,adiff[hlo:nx+hlo,hlo:ny+hlo],'adiff')
dn.dnami_io.globalMinMax(dtree,bdiff[hlo:nx+hlo,hlo:ny+hlo],'bdiff')
if dMpi.ioproc:
