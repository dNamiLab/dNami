...
The ability of \texttt{dNami} to clearly separate the problem statement from its numerical implementation (often a major time sink in research laboratories) is rooted in the flexibility of the Python language from where users can define their own system of balance laws in the form of \autoref{eq:gov_eq}, which is then interpreted to build a computationally-efficient library callable from Python. Users can thus easily interact with their own system of balance laws including at runtime, thereby making it possible to easily integrate solutions to \autoref{eq:gov_eq} with other tools, such as optimisation tools, seamlessly from small to large scale calculations.
