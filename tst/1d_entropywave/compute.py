...
# -- Fill with sin wave centered at x= lx/2 
rho[hlo:nx+hlo] = rho0 + eps * ( np.cos( np.pi*(xloc[:]-dn.cst(0.5)*Lx)/Lp ) ) * ( np.abs(xloc[:] - dn.cst(0.5)*Lx) <= dn.cst(0.5)*Lp   )
u  [hlo:nx+hlo] = u0 
...
dn.dnami_io.write_restart(0,dn.cst(0.),1,dtree)
...
if dMpi.ioproc: print("Tend* :",ti*2.0*m.pi)

t1 = timer()

if dMpi.ioproc: print("Time",(t1-t0)/(grid['nxgb']*grid['nygb']*grid['nzgb']*3*nitmax)*mpi['nxpr']*mpi['nypr']*mpi['nzpr'],t1-t0)
